...
import { CalendarHeartIcon, ClockIcon, CopyIcon, Play } from "lucide-react";
...
const RecordingCard = ({ recording }: { recording: CallRecording }) => {
    const handleCopyLink = async () => {
        try {
            ...
            toast.success("Recording link copied to clip board");
        } catch (err) {
            toast.error("Failed to copy link to clip board");
        }
    };

    const formattetdStartTime = recording.start_time ? format(new Date(recording.start_time),"MMM d, yyyy,, hh:mm a") : "Unknown";
    ...
    return (
        <Card className="group hover:shadow-md transition-all">
            ...
            <CardContent>
                <div className="w-full aspect-video bg-muted/50 rounded-lg flex items-center justify-center cursor-pointer group" onClick={() => window.open(recording.url, "_blank")}>
